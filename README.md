...
- **Auto-Sync (optional)** – Mark an account as "Auto update" to keep it synchronized whenever the site mutates its cookies (e.g., refresh tokens).
...
### 3. Switch Accounts
- For the chosen site, click **Switch** next to an account. Cookie Switch clears existing cookies for the domain, sets the stored set, reloads matching tabs, and records the active auto-sync mapping if enabled.
...
- **Full Backup** – The "All Sites Backup" card can back up or restore everything, including the `activeAccountMap` that ties origins to auto-sync accounts.

### 6. Deleting Data
- Removing a site drops all associated accounts and clears auto-sync mappings for that origin.
- Deleting an account that was auto-sync enabled also removes its active mapping.
...
- `background.js` – Service worker that applies cookies, tracks active accounts, and auto-syncs.
- `icons/refresh-cookie-*.png` – Icon set used for the toolbar and Chrome Web Store assets.
...
- **Auto update not triggering** – Ensure the account was switched at least once after enabling auto-sync so the extension knows it is the active profile.
